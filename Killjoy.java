...
public class Killjoy {
    ...
    private static void handleTestCase(BufferedReader br) throws Exception {
        int[] nx = readArrInt(br);
        int x = nx[1];
        int[] A = readArrInt(br);
        System.out.println(getNumberOfRounds(x, A));
    }
    ...
    private static int getNumberOfRounds(int x, int[] A) {
        int diff = 0;
        int containsX = 0;
        ...
        if (containsX == A.length) {
            return 0;
        }
        if (diff == 0 || containsX > 0) {
            return 1;
        } else {
            return 2;
        }
